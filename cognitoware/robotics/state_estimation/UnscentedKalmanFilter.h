...
struct SigmaPoint {
  static std::vector<SigmaPoint<X2, Y>> CreateSigmaPoints(
      const math::probability::continuous::GaussianMoment<X2>& belief, double alpha,
    ...
    std::vector<SigmaPoint<X2, Y>> sigma_pts(2 * n + 1);
    ...
    for (int i = 0; i < n; i++) {
      ...
      sigma_pts[i + 1 + n] = SigmaPoint<X2, Y>(x2, wn, wn);
    }
    return sigma_pts;
  }
  ...
  static math::data::Matrix GetWeightedError(
      const std::vector<SigmaPoint<X2, Y>>& sigmaPts, int n, Y next) {
    using math::data::Matrix;
    using math::data::Vector;

    Matrix next_e(n, n);
    for (int i = 0; i < sigmaPts.size(); i++) {
...
public:
  ...
  Marginalize(
    ...
    std::vector<SigmaPoint<X, X>> sigma_x = SigmaPoint<X, X>::CreateSigmaPoints(
        belief_, alpha_, beta_, lambda());

    for (int i = 0; i < sigma_x.size(); i++) {
    ...
    Matrix next_e = SigmaPoint<X, X>::GetWeightedError(sigma_x, n,
        next_x);
  ...
  BayesianInference(
    ...
    std::vector<SigmaPoint<X, X>> sigma_z = SigmaPoint<X, Z>::CreateSigmaPoints(
        belief, alpha_, beta_, lambda());

    for( int i = 0; i < sigma_z.size(); i++ ) {
...
private:
  double lambda() const {
    double a = alpha_;
    double k = kappa_;
    double n = belief_.order();
    return (a * a) * (n + k) - n;
  }

  math::probability::continuous::GaussianMoment<X> belief_;
